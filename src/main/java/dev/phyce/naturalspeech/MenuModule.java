...
public class MenuModule implements PluginModule {
	...
	private void drawVolumeMenus(MenuEntry[] entries, int menuIndex) {
		...
		for (int index = entries.length - 1; index >= 0; index--) {
			...
			MenuAction type = audioEngine.isMuted() ? MenuAction.RUNELITE : MenuAction.RUNELITE_SUBMENU;
			...
			if (interfaces.contains(groupId)) {
				...
				for (VoiceConfigMenu result : results) {
					if (result.children == null) {
						...
						MenuEntry parent = _drawVoiceMenu(null, result, 1, MenuAction.RUNELITE_SUBMENU);

						for (VoiceConfigMenu child : result.children) {
							Preconditions.checkNotNull(child);

							_drawVoiceMenu(parent, child, 1, MenuAction.RUNELITE);
						}
					}
				}
			}
			...
			if (interfaces.contains(groupId)) {
				...
				for (TabConfigMenu result : results) {
					if (result.children == null) {
						...
						MenuEntry parent = _drawMuteMenu(null, result, 1, MenuAction.RUNELITE_SUBMENU);

						for (TabConfigMenu child : result.children) {
							Preconditions.checkNotNull(child);

							_drawMuteMenu(parent, child, 1, MenuAction.RUNELITE);
						}
					}
				}
			}
